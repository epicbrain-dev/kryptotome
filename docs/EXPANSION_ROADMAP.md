...
- [x] **Browser Extension for Web VTTs (Roll20 & Alchemy)**:
...
- [x] **Open Character Builder Adapters**:
...
- [x] **Standalone "Pocket Vault" Mobile App (Tauri / iOS / Android)**:
...
- [x] **Attribute-Level Selective Disclosure**:
...
- [x] **Collective Party Pooling (Multi-Holder Aggregation)**:
...
- [x] **Passkey & FIDO2 / WebAuthn Hardware Binding**:
...
- [x] **Kryptotome Publisher Studio (GUI Desktop App)**:
...
- [x] **Crowdfunding Fulfillment Bridge (Kickstarter & BackerKit)**:
...
- [x] **Print-on-Demand (POD) Physical Vouchers & NFC Book Tags**:
...
- [x] **Air-Gapped Table Beacons**:
  - Lightweight verifier daemon capable of running on a battery-powered Raspberry Pi or GM laptop (`AirGappedTableBeaconDaemon`, `table-beacon` CLI, and `@kryptotome/sdk/beacon`).
...
- [x] **Organized Play & Tournament Fast Check-In**:
  - Specialized scanner app for convention tournament check-ins (e.g., Pathfinder Society, Adventurers League).
  - Verify character sheet build legality and official rulebook ownership in $< 10\text{ms}$ via QR code scan with zero personal identity revealed (`TournamentCheckInTicket`, `TournamentScanner`, Pocket Vault tournament ticket).
