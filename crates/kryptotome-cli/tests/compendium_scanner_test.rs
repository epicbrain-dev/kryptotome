...
fn setup_test_compendium() -> (tempfile_guard::TempDirGuard, std::path::PathBuf) {
    let temp_dir =
        std::env::temp_dir().join(format!("ktome_compendium_test_{}", rand::random::<u64>()));
...
fn test_compendium_scanner_deterministic_sha256() {
    ...
    assert_eq!(
        find_entry("rules/combat.json").content_type,
        "application/json"
    );
    assert_eq!(
        find_entry("assets/tokens/red_dragon.png").content_type,
        "image/png"
    );
    assert_eq!(
        find_entry("maps/dungeon_4k.webm").content_type,
        "video/webm"
    );
    assert_eq!(
        find_entry("audio/dungeon_ambience.ogg").content_type,
        "audio/ogg"
    );
...
fn test_compendium_scanner_nonexistent_directory() {
    let bogus_dir =
        std::env::temp_dir().join(format!("nonexistent_ktome_{}", rand::random::<u64>()));
...
fn test_verify_package_manifest_valid() {
    ...
    let report_explicit =
        kryptotome_cli::publisher::verify_package_manifest(&manifest, Some(&pubkey_hex)).unwrap();
...
fn test_verify_package_manifest_mismatching_pubkey_fails() {
    ...
    let err =
        kryptotome_cli::publisher::verify_package_manifest(&manifest, Some(&other_pubkey_hex));
    assert!(err.is_err());
}
...
fn test_verify_package_directory_integrity() {
    ...
    let pristine_report =
        kryptotome_cli::publisher::verify_package_directory(&manifest, &temp_dir, false).unwrap();
    ...
    assert!(tampered_report
        .altered_files
        .contains(&"rules/combat.json".to_string()));
}
...
fn test_license_metadata_validation_orc_cc_by_cc0() {
    ...
    let report_orc =
        kryptotome_cli::publisher::verify_package_manifest(&manifest_orc, None).unwrap();
    assert_eq!(report_orc.license.r#type, "ORC-1.0");

    // 2. Creative Commons CC-BY-4.0 validation
    let cc_by_license = kryptotome_cli::publisher::PackageLicense::new_cc_by(
        "Attribution: 5e SRD Creative Commons CC-BY-4.0",
    );
    let manifest_cc_by = toolchain
        .build_and_sign_package_with_license(
            "wizards.srd5",
    ...
    let report_cc_by =
        kryptotome_cli::publisher::verify_package_manifest(&manifest_cc_by, None).unwrap();
    ...
    let report_cc0 =
        kryptotome_cli::publisher::verify_package_manifest(&manifest_cc0, None).unwrap();
