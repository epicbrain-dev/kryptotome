...
fn bench_zk_verification(c: &mut Criterion) {
    ...
    let challenge = ChallengeNonce::new(package_id.to_string(), challenge_nonce, 300);
...
fn bench_zk_proving(c: &mut Criterion) {
    ...
    let challenge = ChallengeNonce::new(package_id.to_string(), proving_nonce, 300);
...
fn bench_cache_lookup(c: &mut Criterion) {
    ...
    verifier
        .cache_mut()
        .mark_verified(package_id, "sha256:digest");
